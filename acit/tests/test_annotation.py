...
def test_annotation3():
    ...
    assert '2C-2' not in annotation['rules'] or annotation['rules']['2C-2'] is False


def test_annotation4():
    # TBX5基因3'端，仅覆盖UTR区
    annotation = annotate.annotate('chr12', 114737220, 114793000, 'del')
    assert '2D-1' not in annotation['rules'] or annotation['rules']['2D-1'] is False
...
def test_annotation12():
    ...
    assert annotation['rules']['2A']
    assert '2H' not in annotation['rules'] or annotation['rules']['2H'] is False
